...
export const getPopularMovies = async (type) => {
    try{
        ...
        console.error('Error fetching pupular movies:', error.response?.data || error.message);
        throw error;
    }
}
...
export const getMovieVideos = async (movieId, type) => {
    try {
      ...
      return response.data.results.filter(video => video.type === 'Trailer' && video.site === 'YouTube');
    } catch (error) {
